...
Artifact produced from this file will be in `plot_figures`, `models` and `logs` folders.
"""


# Import necessary libraries as shown in cell 1 of the notebook.
...
from features import plot_features, cat_features, final_features

#features to keep

# plot_features = ["Churn", "Customer_Age", "Marital_Status", "Total_Trans", "Heatmap"]
#
# cat_features = [
#     'Gender',
#     'Education_Level',
...
def import_data(dataset_path):
    """
    returns dataframe for the csv found at dataset path
    ...
    #Create a new column named "Churn" using "Attrition_Flag".
    dataframe["Churn"] = dataframe.Attrition_Flag.apply(
        lambda val: 0 if val == "Existing Customer" else 1)
    return dataframe


def visualize_eda(dataframe):
    """
    group all the plots into one method called visualize
    perform eda on df via plotting and save figures to folder called plot_figures
    input:
            dataframe: pandas dataframe
    ...
    for column_name in plot_features:
        ...
        plt.savefig("plot_figures/eda/%s.jpg" % column_name)
        plt.close()
...
def feature_engineering(dataframe):
    ...
    x_df = pd.DataFrame()
    x_df[final_features] = dataframe[final_features]
...
def classification_report_image(data):
    """
    produces classification report for training and testing results and stores report as a figure
    in plot_figures folder
    input:
    ...
    for title, classification_data in classification_reports_data.items():
        ...
        plt.text(
            0.01, 0.7, str(
                ...
                "fontsize": 10}, fontproperties="monospace")
        plt.axis("off")
        plt.savefig("plot_figures/results/%s.jpg" % title)
        plt.close()
...
def feature_importance_plot(model, x_data, output_pth):
    ...
    plt.savefig("plot_figures/%s/Feature_Importance.jpg" % output_pth)
    plt.close()


def train_models(x_train, x_test, y_train, y_test):
    """
    train models with grid search, generate classification reports and store model results
    input:
              x_train: X training data
              x_test: X testing data
              y_train: y training data
    ...
    #generate classificcation report
...
if __name__ == "__main__":
    ...
    visualize_eda(data_df)
    encoded_data_df = encoder_helper(data_df,
                                     cat_features)
    x_train_, x_test_, y_train_, y_test_ = feature_engineering(
        encoded_data_df)
